...
impl AABB2D {
    pub fn new(min: Vec2, max: Vec2) -> AABB2D { AABB2D { min, max } }
    ...
    pub fn mul_affine(&self, affine: &Affine2) -> AABB2D {
        AABB2D {
    ...
    pub fn from_circle(mat: &Affine2) -> AABB2D {
        let a = vec2(
    ...
    pub fn from_box(mat: &Affine2) -> AABB2D {
        let corners = [
            vec2(-0.5, -0.5),
            vec2(-0.5, 0.5),
            vec2(0.5, -0.5),
    ...
    pub fn intersect(self, other: AABB2D) -> AABB2D {
        AABB2D {
