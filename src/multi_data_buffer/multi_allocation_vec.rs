...
#[derive(Debug)]
pub struct MulitAllocationVec<T> {
    allocations: Vec<MultiAllocation<T>>,
    pub minimum_allocation_size: usize,
}

#[derive(Debug)]
struct MultiAllocation<T> {
...
impl<T: Copy + Default> AllocatedVec<T> for MulitAllocationVec<T> {
    fn push(&mut self, mut values: &[T], allocator: &mut BuddyBufferAllocator, buffer: &mut Buffer) -> OctaResult<Vec<AllocatedVecIndex>> {
        ...
        if values.len() < capacity {
            self.allocations.push(MultiAllocation {
                allocation,
                start_index,
                padding,
                data,
...
impl<T> Default for MulitAllocationVec<T> {
    fn default() -> Self {
        Self { 
            allocations: vec![],
