...
use nalgebra::Norm;
use octa_force::{itertools::Itertools, log::error, OctaResult};
...
impl<T: Copy + Default> AllocatedVec<T> for SplitAllocatedVec<T> {
    fn push(&mut self, mut values: &[T], allocator: &mut BuddyBufferAllocator) -> OctaResult<Vec<AllocatedVecIndex>> {
        ...
        let (start, size) = allocator.alloc(self.minimum_allocation_size.max(values.len()))?;

        let capacity = size / size_of::<T>();
        let padding = start % size_of::<T>();
        let start_index = start / size_of::<T>() + (padding != 0) as usize; 
