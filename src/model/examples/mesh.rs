...
#[derive(Debug, Clone)]
pub struct ComposeMeshState {
    pub scene: SceneWorkerSend,
    pub scene_dag_key: SceneDAGKey,
}

impl<V2: Ve<T, 2>, V3: Ve<T, 3>, T: Nu> BS<V2, V3, T> for ComposeMeshState {
...
impl ComposeMesh {
    pub fn new(scene: SceneWorkerSend, camera: &Camera, palette: SharedPalette) -> Self {
        let scene_dag_key = SceneDAGKey::null();
        let mut state =  ComposeMeshState {
            scene,
            scene_dag_key,
        }; 
