...
use crate::{multi_data_buffer::buddy_buffer_allocator::BuddyBufferAllocator, new_logic_state, util::{aabb::AABB, math::get_dag_node_children_xzy_i, math_config::MC, number::Nu, vector::Ve}, volume::VolumeQureyPosValue, voxel::dag64::{lod_heuristic::LODHeuristicT, node::VoxelDAG64Node}};

use super::{DAG64Entry, DAG64EntryKey, ParallelVoxelDAG64, VoxelDAG64};

impl<LOD: LODHeuristicT> ParallelVoxelDAG64<LOD> {
    pub fn update_pos_query_volume<V: Ve<T, 3>, T: Nu, M: VolumeQureyPosValue<V, T, 3> + Send + Sync>(
        &mut self, 
        model: &M,
    ...
    fn update_pos_recursive_par<V: Ve<T, 3>, T: Nu, M: VolumeQureyPosValue<V, T, 3> + Send + Sync>(
        &self, 
        model: &M, 
        aabb: AABB<V, T, 3>, 
        node_level: u8, 
