...
use crate::{multi_data_buffer::buddy_buffer_allocator::BuddyBufferAllocator, util::{aabb3d::AABB, iaabb3d::AABBI, math::get_dag_node_children_xzy_i}, volume::VolumeQureyPosValueI, voxel::dag64::node::VoxelDAG64Node};

use super::{DAG64EntryData, DAG64EntryKey, ParallelVoxelDAG64, VoxelDAG64};


impl ParallelVoxelDAG64 {
    pub fn update_pos_query_volume<M: VolumeQureyPosValueI + Send + Sync>(
        &mut self, 
        model: &M,
        ...
        let root = self.update_pos_recursive_par(model, model_aabb, entry_data.levels, entry_data.offset, entry_data.root_index)?;
    ...
    fn update_pos_recursive_par<M: VolumeQureyPosValueI + Send + Sync>(
        &self, 
        model: &M, 
        aabb: AABBI, 
        node_level: u8, 
        ...
        let children: SmallVec<[_; 64]> = self.nodes.get_range(node.range()).to_smallvec();
        let bitmask = node.pop_mask;

        let (new_children, new_bitmask) = get_dag_node_children_xzy_i().into_par_iter()
            .enumerate()
            .map(|(i, pos)| {
            ...
            .map(|(i, node_aabb)| {
                ...
                let new_node = if !node.is_occupied(i as u32) {
                    ...
                    (self.add_pos_query_recursive_par(
                        model, 
                        node_aabb.min,
                        new_level,
                    )?.check_empty(), true)
                } else if aabb.contains_aabb(node_aabb) {
                    (Some(self.add_pos_query_recursive_par(
                        model, 
                        node_aabb.min,
                        new_level,
                    )?), false)
                } else {
                    (Some(self.update_pos_recursive_par(
                        model,
                        aabb,
                        new_level,
                        node_aabb.min,
            ...
            .try_fold(|| (children.clone(), bitmask), 
            ...
            .try_reduce(|| (children.clone(), bitmask), 
