...
use crate::{multi_data_buffer::buddy_buffer_allocator::BuddyBufferAllocator, new_logic_state, util::{aabb3d::AABB, iaabb3d::AABBI, math::get_dag_node_children_xzy_i}, volume::VolumeQureyPosValueI, voxel::dag64::node::VoxelDAG64Node};
...
impl ParallelVoxelDAG64 {
        ...
        let (new_children, new_bitmask) = get_dag_node_children_xzy_i()
            ...
            .map(|(i, node_aabb)| {
                ...
                let new_node = if !node.is_occupied(i as u32) {
                    ...
                    (self.add_pos_query_recursive_lower(
                        model, 
                        node_aabb.min,
                        new_level,
                    )?.check_empty(), true)
                } else if aabb.contains_aabb(node_aabb) {
                    (Some(self.add_pos_query_recursive_lower(
                        model, 
                        node_aabb.min,
                        new_level,
                    )?), false)
                } else {
                    (Some(self.update_pos_recursive_lower(
                        model,
                        aabb,
                        new_level,
                        node_aabb.min,
    ...
    fn update_pos_recursive_lower<M: VolumeQureyPosValueI>(
        &self, 
        model: &M, 
        aabb: AABBI, 
        node_level: u8, 
        ...
        for (i, pos) in get_dag_node_children_xzy_i().into_iter()
            ...
            if aabb.collides_aabb(node_aabb) {
                ...
                if !node.is_occupied(i as u32) {
                    ...
                    let new_child_node = self.add_pos_query_recursive_lower(
                        model, 
                        min,
                        new_level,
                    )?;
                ...
                let new_child_node = if aabb.contains_aabb(node_aabb) {
                    ...
                    self.update_pos_recursive_lower(
                        model,
                        aabb,
                        new_level,
                        min,
