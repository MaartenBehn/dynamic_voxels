...
use crate::{util::aabb3d::AABB, volume::VolumeQureyPosValue, voxel::palette::palette::MATERIAL_ID_NONE};
...
impl CSGTree<u8> {
    fn get_pos_internal(&self, pos: Vec4, index: CSGTreeKey) -> u8 {
        ...
        match &node.data {
            ...
            CSGNodeData::All(v) => *v,
            CSGNodeData::OffsetVoxelGrid(voxel_grid) => voxel_grid.get_value(Vec3A::from(pos)),
            CSGNodeData::SharedVoxelGrid(shared_voxel_grid) => shared_voxel_grid.get_value(Vec3A::from(pos)),
        }
    }
}
