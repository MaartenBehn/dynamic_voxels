...
impl ShaderProfiler {
    pub fn new(
        ...
        debug!(
            "Profiler Buffer size: {} MB",
            profiler_size as f32 / 1000000.0
        );
        ...
        let egui_descriptor_layout = context
            .create_descriptor_set_layout(&[vk::DescriptorSetLayoutBinding {
                binding: 0,
                descriptor_count: 1,
        ...
        for _ in 0..3 {
            ...
            let (result_image, result_staging_buffer) =
                context.create_live_egui_texture_image(format, sample_res)?;

            egui_descriptor_set.update(&[WriteDescriptorSet {
                binding: 0,
    ...
    fn get_sample_res(res: UVec2) -> (UVec2, u32) {
        ...
        (
            uvec2(MAX_SAMPLE_RES_WIDTH, y as u32),
            (1.0 / reduction_factor) as u32,
        )
    }
    ...
    pub fn process_results(&mut self, frame_index: usize, context: &Context) -> OctaResult<()> {
        let pixel_index =
            (self.active_sample_pixel.y * self.sample_res.x + self.active_sample_pixel.x) as usize;
        ...
        let total_end =
            data[self.main_scope * 5 + 3] as u64 + (data[self.main_scope * 5 + 4] as u64) << 32;
        ...
        for (i, name) in self.scopes.iter().enumerate() {
            ...
            self.pixel_results[pixel_index].scope_data[i]
                .add_mean_calls(counter as f32, sample_count as f32);
            self.pixel_results[pixel_index].scope_data[i]
                .add_mean_percent(percent, sample_count as f32);

            self.mean_pixel_results.scope_data[i]
                .add_mean_calls(counter as f32, global_sample_count as f32);
            self.mean_pixel_results.scope_data[i]
                .add_mean_percent(percent, global_sample_count as f32);
        }
        ...
        *self.last_colors.last_mut().unwrap() = color_u8;
        ...
        context.copy_live_egui_texture_staging_buffer_to_image(
            &self.result_overview_images_staging_buffers[staging_result_image],
            &self.result_overview_images[staging_result_image].image,
        )?;
    ...
    pub fn gui_windows(&mut self, ctx: &egui::Context, mouse_left: bool) {
        ...
        egui::Window::new("Shader Profile").show(ctx, |ui| {
            ...
            let image =
                egui::Image::new(self.result_overview_textures[self.current_result_overview_image])
                    .shrink_to_fit();
            ...
            if let Some(Pos2 { x, y }) = ui.ctx().pointer_latest_pos() {
                ...
                if pos.x > 0.0
                    && pos.x < self.sample_res.x as f32
                    && pos.y > 0.0
                    && pos.y < self.sample_res.y as f32
                {
    ...
    pub fn on_recreate_swapchain(
        ...
        for i in 0..3 {
            ...
            self.egui_descriptor_sets[i].update(&[WriteDescriptorSet {
                binding: 0,
                kind: WriteDescriptorSetKind::CombinedImageSampler {
                    layout: vk::ImageLayout::GENERAL,
                    view: &result_image.view,
            ...
            let texture = SizedTexture::new(
                self.result_overview_textures[i].id,
                self.sample_res.as_vec2().as_ref(),
            );
